...
public class AvatarController {
    ...
    @Autowired
    public AvatarController(UserServiceImpl userServiceImpl, AvatarService avatarService, ImageService imageService) {
    ...
    public ResponseEntity<List<ImageDTO>> getAvatarsId(){
            return ResponseEntity.ok(avatarService.getUserAvatars(userServiceImpl.getCurrentUser()).stream()
    ...
    public ResponseEntity<List<ImageDTO>> getAvatarsByUserId(@PathVariable("id") int id){
        return ResponseEntity.ok(avatarService.getUserAvatars(userServiceImpl.findOne(id)).stream()
                .map(imageService::convertSpecificImageToImageDTO).toList());
    }
    ...
    public ResponseEntity<HttpStatus> uploadImage(@RequestBody MultipartFile image){
        try {
            avatarService.createAvatar(userServiceImpl.getCurrentUser(), image);
        } catch (IOException e) {
            throw new AvatarNotCreatedException();
        }
    ...
    public ResponseEntity<HttpStatus> deleteAvatar(@PathVariable("id") int id){
        if (avatarService.getUserAvatars(userServiceImpl.getCurrentUser()).stream()
