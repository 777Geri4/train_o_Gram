...
public class PostController {
    ...
    @Autowired
    public PostController(PostServiceImpl postServiceImpl, UserServiceImpl userServiceImpl, LikePostServiceImpl likePostServiceImpl) {
    ...
    public ResponseEntity<List<PostDTO>> getPosts() {
        return ResponseEntity.ok(postServiceImpl.findAllUserPost(userServiceImpl.getCurrentUser()).stream()
                .map(postServiceImpl::convertPostToPostDTO).toList());
    }

    @GetMapping("/recommendations")
    public ResponseEntity<List<PostDTO>> getRecommendedPosts() {
        return ResponseEntity.ok(postServiceImpl.findSuggestions(userServiceImpl.getCurrentUser()).stream()
                .map(postServiceImpl::convertPostToPostDTO).toList());
    }
    ...
    public ResponseEntity<HttpStatus> createPost(@RequestBody MultipartFile image, String message){
        try {
            postServiceImpl.createPost(userServiceImpl.getCurrentUser(), image, message);
        } catch (IOException e) {
